...
export function sendGeneralChatMessage(username, message) {
...
export function createGroupChat(channelName) {
...
export function generatePrivateChatTopic(user1, user2) {
...
export function sendPrivateMessage(sender, recipient, message) {
...
export function generateGroupChatTopic(channelName) {
    return `group-chat/${channelName}`;
}

export function joinGroupChat(username, channelName) {
    if (!registeredUsernames.has(username)) {
...
export function inviteToGroupChat(inviter, invitee, channelName) {
...
export function sendGroupChatMessage(username, channelName, message) {
    if (!registeredUsernames.has(username)) {
        console.log(`L'utilisateur '${username}' n'est pas enregistré.`);
        return;
    }
...
export function leaveGroupChat(username, channelName) {
    if (!registeredUsernames.has(username)) {
        console.log(`L'utilisateur '${username}' n'est pas enregistré.`);
        return;
    }
